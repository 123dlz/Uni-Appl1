<template>
	<view>
		...
		<view class="view" v-if="jiaod">
			...
			<view>
				<text class="tit"
					style="color: #ff60a3;font-size: 30rpx;font-weight: 600;padding-top: 15rpx;">热门搜索</text>
				<view>
		...
		<view class="list" v-for="(item,index) in infolist" :key="index.id" @tap="play(item)"
			style="margin-top: 140rpx;">
			<image :src="url+item.img" mode="aspectFill" style="margin-left: 20rpx;"></image>
			<view class="tag">已更新至第{{item.num}}集</view>
			<view style="margin-top: 20rpx;margin: 20rpx;">
				<text class="font vname">{{item.class}}:{{item.title}}</text>
			</view>
			<view style="margin-top:3rpx;margin: 20rpx;">
				<text style="color: #4f5684;font-size:28rpx ;">{{item.text}}</text>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			getdata(is) {
				...
				this.GetJson(this.ip + 'souduanju.php', 'POST', {
					info: this.value,
			...
			play(item) {
				uni.navigateTo({
					url: '../duanju/play-apple?id=' + item.id
				})
			}
		},
		onReachBottom() {
