			...
			<view class="servv" >
					<view class="list row" v-for="item in infolist" :key="item.id" @tap="play(item)">
						...
						<view>
							...
							<text style="font-size: 23rpx;color: #866EF9;;margin-top: 5rpx;">{{gettime(item.vod_time_add,'yyyy-MM-dd hh:mm')}}</text>
	...
	export default {
		...
		methods: {
			...
			getdata(is){
				...
				this.GetJson(this.ip+'soudata.php','POST',{
					info:this.value,
			...
			play(item){
				if(this.type==1){
					uni.navigateTo({
						url: '../play/play?data='+ item 
						+ '&play='+ item.vod_play_url
						+'&zuanshi='+ item.vod_points_play
