<template>
	<view>
		...
		<view style="margin-top:130rpx;">
			<view class="list" v-for="(item,index) in infolist" :key="index.id" @tap="play(item.id,0)">
				<view class="row">
					<view style="width: 200rpx;">
						<image class="image" :src="url + item.img" mode="aspectFill" style="margin-left: 20rpx;"></image>
					</view>
					<view style="padding:0 40rpx">
						<text class="font vname">{{item.name}}</text>
						<view class="vodinfo" style="color: #FFF;font-size:28rpx ;">{{item.info}}
						</view>
					</view>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			play(id,index) {
				uni.navigateTo({
					url: 'play?id=' + id + '&courseDetailsId=1'
				})
			},
		},
		onReachBottom() {
